import { View, Text, ScrollView, StyleSheet, TouchableOpacity, useEffect, useState } from 'react-native';
          ...
          <Card key={index} style={styles.statCard} padding="small">
            <View style={styles.statHeader}>
              ...
              <Text style={styles.statValue}>
                {stat.format === 'currency' ? config.formatCurrency(stat.value) : stat.value}
              </Text>
            </View>
      ...
      <Card style={styles.section} padding="medium">
        ...
        <View style={styles.actionsGrid}>
          ...
          <QuickAction
            icon="person-add"
            label="Add Client"
            onPress={() => router.push('/clients/edit')}
          />
          <QuickAction
            icon="description"
        ...
        {brief?.items && brief.items.length > 0 ? (
          brief.items.map((item, idx) => (
            <View key={idx} style={styles.scheduleItem}>
              <MaterialIcons name={getItemIcon(item.type)} size={20} color={config.theme.textSecondary} />
  ...
  return (
    <TouchableOpacity style={styles.quickAction} onPress={onPress}>
      <View style={styles.quickActionIcon}>
        <MaterialIcons name={icon} size={28} color={config.theme.primary} />
      </View>
